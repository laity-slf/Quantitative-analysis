...
class my_stream():
    ...
    def val(self,order_data,trade_data):
        ...
        for i in range(0,self.index):
            ...
            #用先后来判断 i
                ...
                a=[self.stream[i][1]]+self.stream[i][12:]
                if a!=self.T[tindex]:
                    m_T[tindex]=a
                tindex+=1
    ...
    def insertSort(self,price):
        self.bid_first.append(price)
        self.bid_first.sort()

    def get_bid_first(self):
        '''
        去除末端 balance=0 的price;
        小于0的情况只能出现在po为假的时候,这个时候如果出现，我们考虑买单已经匹配，也就是已经被消耗,不参与后续买一的计算
        若后续真的po来到会让通过update_date进入排序
        '''
        while self.bid_pb[self.bid_first[-1]]<=0:
            self.bid_first.pop()
        return self.bid_first[-1]

    def update_bid(self,_price,_balance,type='T'):
        if type =='F':_balance=-_balance
        if _price in self.bid_pb :
            # 防止假的po balance <0 被去除
            if _price not in self.bid_first:
                self.insertSort(_price)
            self.bid_pb[_price] += _balance 
        else:
            self.bid_pb[_price] = _balance
            self.insertSort(_price)
    ...
    def insert_for_order(self,addline):
        ...
        if addline[6]=='D':
            ...
            #减少对应价格的balance
            if addline[5]=='B':
                self.bid_pb[addline[3]]-=addline[4]
            self.index += 1
        else:
            if addline[2] in self.fakepo:
                self.stream.pop(self.temppo[addline[2][0]])
            else:
                # 不在 fake 更新对应的买一价格
                if addline[5] == 'B' :
                    if addline[3]==8.6:
                        print('1')
                    self.update_bid(addline[3],addline[4])
            tag,index = self.find_last_T_PO()
        #PO不匹配，直接插在后面因为在外部已经用了归并保证时序
            if tag==-1:
                ...
                index = max(index,self.temppo[addline[2]][1])
    ...
    def insert_for_trade(self,addline):
        ...
        if addline[9]=='N':
            ...
            #集时竟价是 设定是一定会有与之对应的po
            # 先取再减，保证当前ao对应的po也参与
            bid_first_price = self.get_bid_first()
            self.update_bid(addline[2],addline[3],type='F')
            self.stream.append(['T', addline[0], bid_first_price ,0, 0, 0, 0, 0, 0, 0, 0, 0, addline[1], addline[2], addline[3]
                                       , addline[4], addline[5], addline[6], addline[7], addline[9], addline[10]])
            self.index +=1 

        if addline[9]=='S':
            self.T.append(addline[0:8]+addline[9:])
            #TODO 更新买一价格,这边是主动卖，所以取之前被动买的最大值,但是存在po单还没构建的情况：1求max2.进去if里面算，这边采用2
            # bid_first_price = max(addline[2],self.bid_first[-1])
            
            # 1.先找到对应的PO 然后active 从po开始寻找对应的ao
            ...
            if act_match >= 0 and pass_match >= 0:
                ...
                self.stream.insert(T_index,['T', addline[0], bid_first_price ,0, 0, 0, 0, 0, 0, 0, 0, 0, addline[1], addline[2], addline[3]
                                       , addline[4], addline[5], addline[6], addline[7], addline[9], addline[10]])
            ...
            if act_match < 0 and pass_match >= 0:
                ...
                self.stream.insert(T_index+1,['T', addline[0], bid_first_price,0, 0, 0, 0, 0, 0, 0, 0, 0, addline[1], addline[2], addline[3]
                                       , addline[4], addline[5], addline[6], addline[7], addline[9], addline[10]])
        ...
        if addline[9]=='B':
            self.T.append(addline[0:8]+addline[9:])
            #TODO 更新买一价格,这边是主动买，所以取当前ao买单与之前po买单的最大值
            bid_first_price = max(self.get_bid_first(),addline[2])
